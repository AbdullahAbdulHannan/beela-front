...
export default function ChangePasswordScreen({ navigation }) {
  ...
  const allValid = current.length > 0 && checks.length && checks.upper && checks.lower && checks.number && checks.special && checks.match;
  ...
  const handleSubmit = async () => {
    ...
    try {
      setIsLoading(true);
      const res = await changePassword({ currentPassword: current, newPassword: password });
      setMessage(res?.message || 'Password changed successfully.');
      setTimeout(() => navigation.goBack(), 900);
    } catch (e) {
      setError(typeof e === 'string' ? e : 'Failed to change password');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <KeyboardAvoidingView style={{ flex: 1, backgroundColor: '#F9FAFB' }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <SafeAreaView style={{ backgroundColor: '#F9FAFB' }} edges={['top']}>
        <View style={styles.header}>
          <TouchableOpacity onPress={() => navigation.goBack()} activeOpacity={0.7} style={styles.headerBack}>
...
const styles = StyleSheet.create({
  container: { flexGrow: 1, backgroundColor: '#F9FAFB', paddingHorizontal: 30, paddingTop: 10, paddingBottom: 40 },
  header: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', padding: 16 },
  headerBack: { width: 40, height: 40, alignItems: 'center', justifyContent: 'center' },
  headerTitle: { fontSize: 20, fontWeight: '700', color: '#1A1D2E' },
  inputContainer: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#FFF', borderRadius: 15, height: 55, marginTop: 15, paddingHorizontal: 15, borderWidth: 1, borderColor: '#E5E7EB' },
  inputIcon: { marginRight: 10 },
  inputField: { flex: 1, fontSize: 16, color: '#333' },
  rulesBox: { backgroundColor: '#fff', borderRadius: 12, borderWidth: 1, borderColor: '#E5E7EB', padding: 12, marginTop: 12 },
  ruleRow: { flexDirection: 'row', alignItems: 'center', marginVertical: 3 },
  ruleText: { marginLeft: 8, color: '#6B7280' },
  primaryButton: { backgroundColor: ACTIVE_COLOR, marginTop: 16, paddingVertical: 18, borderRadius: 15, alignItems: 'center' },
